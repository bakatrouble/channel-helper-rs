...
use crate::database::schema::{Post as PostSchema, UploadTask as UploadTaskSchema, PostMessageIds as PostMessageIdsSchema};
...
impl Database {
    ...
    pub async fn create_post(&self, post: Post) -> anyhow::Result<()> {
        self.conn.call(move |conn| {
            ...
            let mut query_builder =  statement
    ...
    pub async fn add_message_id_for_post(&self, post_id: Uuid, message_id: PostMessageId) -> anyhow::Result<()> {
        self.conn.call(move |conn| {
            let (sql, values) =
                Query::insert()
                    .into_table(PostMessageIdsSchema::Table)
        ...
        Ok(self.conn.call(move |conn| {
            let (sql, values) =
                Query::select()
                    ...
                    .expr_as(Expr::col((PostMessageIdsSchema::Table, PostMessageIdsSchema::ChatId)), Alias::new("chat_id"))                    .column((PostMessageIdsSchema::Table, PostMessageIdsSchema::ChatId))
                    .expr_as(Expr::col((PostMessageIdsSchema::Table, PostMessageIdsSchema::MessageId)), Alias::new("message_id"))
    ...
    pub async fn fetch_unsent_post_with_media_type(&self, media_type: MediaType) -> anyhow::Result<Option<Post>> {
        Ok(self.conn.call(move |conn| {
            let (sql, values) =
                Query::select()
                    .columns([
